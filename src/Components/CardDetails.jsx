  ...
  useEffect(() => {
    const getData = async () => {
      let r = await axios.get(
        `https://red-violet-salmon-wrap.cyclic.app/${type}?_page=${page}&_limit=4`
      );
      setData(r.data);
        ...
        <div className={styles.subContainer}>
          ...
          <div style={{ border: "1px solid #CCD5D6" }} className={styles.bold}>
            ...
            <button style={{ cursor: "pointer" }}>
              <a href="\sell">Make an offer</a>
            </button>
          </div>
          <div>
            <p className={styles.pa}>{address}</p>
            <img
              src="https://www.sparxsys.com/sites/default/files/sampleMap.JPG"
              alt=""
            />
          </div>
        </div>
